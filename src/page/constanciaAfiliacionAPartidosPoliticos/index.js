...
function ConstanciaAfiliacionAPartidosPoliticos (props) {
	const classes = useStyles();
	const titleModal = 'Nueva solicitud'
	const subTitleModal = 'Constancia de afiliación a partidos políticos'
	const contentModal = 'Para generar una nueva solicitud de constancia de afiliación a partidos políticos debe dar clic en el botón "ENVIAR SOLICITUD"'
	...
	const controller = "constanciaAfiliacionAPartidosPoliticos";
	...
	const DescargarConstancia = async (idRegister) =>{
		setLoadingMessage({loading: true, title: 'Descargando constancia de afiliación a partidos políticos ...'})
		...
		try {
			...
			if (response.error) showMessagePersonalizedPosition('warning', '¡Advertencia!', response.error?.message, 'center')
				...
				downloadPdfV2(response.data.base64, 'Constancia de afiliación a partidos políticos.pdf')
			}
		} catch (exception) {
	...
	const FuncionObtenerListadoDeConstanciasDeAfiliados = async () => {
		setLoadingMessage({loading: true, title: 'Cargando constancias de afiliación a partidos políticos ...'})
		...
		try {
			...
			if (hasPermission.error) {
				...
				props.CONSTANCIAAFILIACIONAPARTIDOSPOLITICOS_SET_DATA(data);
	...
	const FuncionSolcitarUnaNuevaConstancia = async () =>{
		setLoadingMessage({loading: true, title: 'Solicitando nueva constancia de afiliación a partidos políticos ...'})
		
		try {
			let response = await ApiServices.constanciaAfiliacionAPartidosPoliticos.createRegister({});

			if (response.error) showMessagePersonalizedPosition('warning', '¡Advertencia!', response.error?.message, 'center')
			else  {
				FuncionObtenerListadoDeConstanciasDeAfiliados()
				FuncionCerrarModalParaNuevaConstancia()
				showMessagePersonalizedPosition('success', '¡Bien!', 'Constancia de afiliación a partidos políticos solicitada correctamente', 'center')
			}
		} catch (exception) {
			showMessagePersonalizedPosition('warning', '¡Advertencia!', 'Intente de nuevo', 'center')
		}
	...
	return (
		<div className={classes.containerPrincipal}>
			...
			/>
			<Title title="Constancia de afiliación a partidos políticos" icon={"/menu/Constancia.png"}/> <br/>
			{
				(authorized.show) ? 
					<Table
...
}export default connect(mapStateToProps, mapDispatchToProps)(ConstanciaAfiliacionAPartidosPoliticos);