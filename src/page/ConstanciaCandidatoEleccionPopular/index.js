...
function ComponenteConstanciaCandidatoEleccionPopular (props) {
	const classes = useStyles();
	const titleModal = 'Nueva solicitud'
	const subTitleModal = 'Constancia de Postulación a Cargo de Elección Popular'
	const contentModal = 'Para generar una nueva solicitud de Constancia de Postulación a Cargo de Elección Popular debe dar clic en el botón "ENVIAR SOLICITUD"'
	...
	const controller = "constanciaCandidatoEleccionPopular";
	...
	const DescargarConstancia = async (idRegister) =>{
		setLoadingMessage({loading: true, title: 'Descargando Constancia de Postulación a Cargo de Elección Popular ...'})
		...
		try {
			...
			if (response.error) showMessagePersonalizedPosition('warning', '¡Advertencia!', response.error?.message, 'center')
				...
				downloadPdfV2(response.data.base64, 'Constancia de Postulación a Cargo de Elección Popular.pdf')
			}
		} catch (exception) {
	...
	const FuncionObtenerListadoDeConstanciasDeAfiliados = async () => {
		setLoadingMessage({loading: true, title: 'Cargando Constancias de Postulación a Cargo de Elección Popular ...'})
		...
		try {
			...
			if (hasPermission.error) {
				...
				props.CONSTANCIACANDIDATOELECCIONPOPULAR_SET_DATA(data);
	...
	const FuncionSolcitarUnaNuevaConstancia = async () =>{
		setLoadingMessage({loading: true, title: 'Solicitando nueva Constancia de Postulación a Cargo de Elección Popular ...'})
		...
		try {
			...
			if (response.error) showMessagePersonalizedPosition('warning', '¡Advertencia!', response.error?.message, 'center')
				...
				showMessagePersonalizedPosition('success', '¡Bien!', 'Constancia de Postulación a Cargo de Elección Popular solicitada correctamente', 'center')
			}
		} catch (exception) {
			showMessagePersonalizedPosition('warning', '¡Advertencia!', 'Intente de nuevo', 'center')
		}
	...
	return (
		<div className={classes.containerPrincipal}>
			...
			/>
			<Title title="Constancia de Postulación a Cargo de Elección Popular" icon={"/menu/Constancia.png"}/> <br/>
			{
				(authorized.show) ? 
					<Table
...
}export default connect(mapStateToProps, mapDispatchToProps)(ComponenteConstanciaCandidatoEleccionPopular);