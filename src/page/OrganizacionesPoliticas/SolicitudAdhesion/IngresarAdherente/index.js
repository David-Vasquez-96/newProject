...
export default function IngresarAdherente(props) {
    /********** VARIABLES **********/
    ...
    const validateFoil = async () => {
        try {
            ...
            if (hasPermission.error) {
                Alert.error("NO TIENE AUTORIZACIÓN PARA OBTENER LA INFORMACIÓN")
                setAuthorized(false);
                setLoading(false);
            } else {
                let newData = [];
                ...
                if (response.error !== null) {
                    ...
                    if (response.data.estadoHojaAfiliacion.descripcion === "DISPONIBLE") {
                        ...
                        Alert.success('HOJA VALIDA, PUEDE AGREGAR ADHERENTES');
    ...
    return (
        <div>
            ...
            {openDeleteModal && <ConfirmationElement open={openDeleteModal} handleClose={handleCloseDeleteModal} handleConfirm={deleteAffiliate}
                titleModal='Eliminar adherente' subtitleModal={'¿Quiere eliminar la linea ' + noLinea + ' de la hoja ' + correlativo + '?'} />}
            ...
            <div>
                {props.faseOpData.faseOp && (props.faseOpData.faseOp != 3 &&
                    (<CardInput valueInput={correlativo} valueChangeInput={setCorrelativo} searchButton={validateFoil} clearButton={clearAllRegister} />)
                )}
                <Table title={"Listado de solicitudes"} header={titleHeader} data={data} setData={setData} options={tableOptions}
